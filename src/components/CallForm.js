...
const CallForm = () => {
  ...
  const {formObj, handleCheckboxChange, handleOnSubmit} = useContext(CallFormContext) 
 
  return (
    <Wrapper className='row mb-5'>
      <div className=' col-12 mb-5'>
        <Header>Call Form</Header>
        <Form onSubmit={handleOnSubmit} id="callForm" >
          ...
          <div className="form-group row">
            <div className="col-sm-10">
              <button type="button" className="btn btn-outline-secondary" id="history-button" 
          ...
          <CallHistoryTable className={ formObj.showCallHistory ? "table": "d-none" }>
            <thead>
              <tr>
                <th>Call Date</th>
                <th>Call Type </th>
          ...
          <CallTypeWrapper className="p-4 mt-4">
                ...
                return (
                  <div key={callType.id+callType.type} className={"col-md-6 form-check mb-1 order-col-"+callType.id}>
                      <input type="checkbox" className="form-check-input" id={callType.id+callType.type}
                        checked={formObj.selectedCallTypes.includes(callType.type)}  
            ...
            <CallNotes className="form-group mt-4 p-2">
              ...
              <textarea className="form-control" id="callNotes" rows="4" onChange={(e)=> formObj.setCallNotes(e.target.value)} value={formObj.callNotes}></textarea>
            </CallNotes>
          </CallTypeWrapper>
                        ...
                        <div key={resource.id+resource.resource} className="row  form-check mb-2">
                          <div className="col">
                            <input type="checkbox" className="form-check-input" id={resource.id+resource.resource}
                              checked={formObj.selectedDigitalResources.includes(resource.resource)}  
                    ...
                    resources.map((resource) => {
                      ...
                      return (
                        <div key={resource.id+resource.resource} className="row form-check mb-2">
                          <div className="col">
                            <input type="checkbox" className="form-check-input" id={resource.id+resource.resource}
                              checked={formObj.selectedPhysicalResources.includes(resource.resource)}  
          ...
          <BlockWrapper className="p-4">
            <label className="form-check-label" htmlFor="blockReason"><h6>REASON FOR BLOCK</h6></label>
              <select className="form-select" aria-label="Reason For Block"  id="blockReason"
                //value={formObj.selectedBlockReason}
                onChange={(e)=> {
                  formObj.setBlockReason(JSON.parse(e.target.value).reason)
          ...
          <ErrorMessage  className=" p-4 mb-5 alert alert-danger" role="alert" id="errorMessage">
          </ErrorMessage>
